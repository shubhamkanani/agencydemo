...
import { BookViewer } from "../../config/BookViewer";
import { features } from "../../config/features";

export function BooksView(props: { id: string }) {
  ...
  if (isLoading) {
    ...
    if (error || document === null) {
      ...
      content = <BooksViewComponent document={document} />;
    }
  }
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.books.view.header")}
...
export function BooksViewComponent(props: { document: Partial<Book> }) {
  const document = props.document;

  const viewer = use(BookViewer, { transient: true });
...
export function getHeaderActions(
  ...
  if (features.edit) {
    actions.push(
      <Link
        key="edit"
        to={router.path(Routes.BOOKS_EDIT, {
          params: { id },
        })}
      >
      ...
      <Ant.Popconfirm
        ...
        onConfirm={() => {
          collection.deleteOne(id).then(() => {
            router.go(Routes.BOOKS_LIST);
            Ant.notification.success({
              message: "Success",
              description: "You have deleted the Book",
            });
          });
        }}
      >
  ...
  const viewRoutePath = router.path(Routes.BOOKS_VIEW, {
    params: { id },
  });
